...
<main id="primary" class="site-main">
  ...
  <?php if (is_page('ve-chung-toi')):
    ...
    if ($tl_group = get_field('timeline')):
      ...
      if ($timelines): ?>
        <?php
          ...
          $group = (array) get_field('timeline', $pid);        // lấy group 'timeline'
          $history_title = trim((string) ($group['history_title'] ?? ''));
          ?>
          <div class="drink-section-title">
            <img class="title-icon" src="<?= $arrow_icon ?>" alt="icon">
            <h2><?= esc_html($history_title ?: 'LỊCH SỬ HÌNH THÀNH') ?></h2>
          </div>
        <div class="container">
      ...
      <?php
        $pid   = get_queried_object_id() ?: get_the_ID();
        $group = (array) get_field('two_column_highlight', $pid);   // group
        $quality_title = trim((string) ($group['quality_title'] ?? ''));
      ?>
      <div class="drink-section-title">
        <img class="title-icon" src="<?= $arrow_icon ?>" alt="icon">
        <h2><?= esc_html($quality_title ?: 'SỨ MỆNH & GIÁ TRỊ CỐT LÕI') ?></h2>
      </div>
    <?php
    ...
    if ($achv = get_field('achievements_tabs')):
      ...
      <?php
        $pid   = get_queried_object_id() ?: get_the_ID();
        $group = (array) get_field('achievements_tabs', $pid);   // lấy group
        $archivement_title = trim((string) ($group['achievements_title'] ?? ''));
      ?>
      <div class="drink-section-title">
        <img class="title-icon" src="<?= $arrow_icon ?>" alt="icon">
        <h2><?= esc_html($archivement_title ?: 'THÀNH TỰU') ?></h2>
      </div>
      <div class="container">
