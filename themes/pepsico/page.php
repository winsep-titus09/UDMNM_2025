  ...
  <div class="content1-group">
    ...
    <?php if (!empty($content1['button_info'])):
      ...
      <a class="custom-btn" href="<?php echo esc_url($button['button_link'] ?? ''); ?>">
        ...
        <img src="<?php echo esc_url($arrow_icon, $allowed_protocols); ?>" alt="" class="btn-icon" loading="lazy" decoding="async">
      </a>
    <?php endif; ?>
  </div>
<?php endif; ?>
  ...
  <div class="stats7-grid">
    <?php for ($i=1;$i<=7;$i++): ?>
      ...
      <?php elseif ($i === 4 || $i === 7): // số + tiêu đề + mô tả ?>
        <div class="stats7-item <?php echo $i===7 ? 'stats7-item--large' : 'stats7-item--number'; ?> box<?php echo (int)$i; ?>">
...
<?php if ($content2 = get_field('content2')): ?>
  ...
  <div class="content2-group">
    <?php if (!empty($content2['title2'])): ?><h2><?php echo esc_html($content2['title2']); ?></h2><?php endif; ?>
...
<?php if (is_front_page() || is_home()): ?>
  ...
  if ($posts_query->have_posts()): ?>
    <div class="post-tabs-row">
      <?php while ($posts_query->have_posts()): $posts_query->the_post(); $meta = (array) get_field('post_box_meta'); ?>
  ...
  if ($query->have_posts()): ?>
    <div class="drink-list" id="drinkCarousel" role="region" aria-label="<?php echo esc_attr__('Sản phẩm', 'pepsico-theme'); ?>">
