...
$grp = get_field('leadership'); // -> array|null
$leaders = [];
if (is_array($grp)) {
  for ($i = 1; $i <= 12; $i++) {
...
<?php if (!empty($leaders)): ?>
  <div class="container mt-5">
    <section class="leaders" aria-label="<?php echo esc_attr__('Đội ngũ lãnh đạo', 'pepsico-theme'); ?>">
      <div class="leaders__grid">
        <?php foreach ($leaders as $ld): ?>
          <article class="leader">
            <?php if ($ld['img']): ?>
              <div class="leader__thumb">
                <img src="<?= esc_url($ld['img']) ?>" alt="<?= esc_attr($ld['name'] ?: __('Lãnh đạo', 'pepsico-theme')) ?>">
              </div>
            <?php endif; ?>
