...
if (!function_exists('spv_resolve_link')) {
  function spv_resolve_link($raw){
    if (is_array($raw))  return $raw['url'] ?? '';
    if (is_numeric($raw)) return get_permalink((int)$raw) ?: '';
      ...
      <?php if ($btn_link_1): ?>
        <a class="custom-btn" href="<?php echo esc_url($btn_link_1); ?>">
          ...
          <span class="spv-newshead__btnIcon" aria-hidden="true">
            <img src="<?php echo esc_url($arrow_icon, $allowed_protocols); ?>" alt="" loading="lazy" decoding="async" class="btn-icon">
          </span>
        </a>
      <?php endif; ?>
    </section>
