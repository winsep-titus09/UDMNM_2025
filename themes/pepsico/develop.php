...
<?php
  ...
  ?>
<section class="spv-hero lazy-bg" <?php if ($bg): ?>data-bg="<?php echo esc_url($bg); ?>" data-bg-var="--spv-hero-bg"
    <?php endif; ?>>
    <?php if ($bg): ?>
    <noscript>
        <style>
        .spv-hero {
            --spv-hero-bg: url('<?php echo esc_url($bg); ?>')
        }
        </style>
    </noscript>
    <?php endif; ?>
...
<section class="ceo-msg mt-5" aria-label="<?php echo esc_attr__('Thông điệp lãnh đạo', 'pepsico-theme'); ?>">
    ...
    <div class="ceo-msg__author">
        <?php if ($ava): ?>
        <img class="ceo-msg__avatar" src="<?php echo esc_url($ava); ?>"
            alt="<?php echo esc_attr($name ?: __('Ảnh đại diện', 'pepsico-theme')); ?>" loading="lazy" decoding="async"
            sizes="96px">
        <?php endif; ?>
        <div class="ceo-msg__meta">
            <?php if ($name): ?>
            <div class="ceo-msg__name"><?php echo esc_html($name); ?></div><?php endif; ?>
            <?php if ($title): ?>
            <div class="ceo-msg__title"><?php echo esc_html($title); ?></div><?php endif; ?>
        </div>
    </div>
    <?php endif; ?>
...
<section id="developHero" class="develop-hero mt-5"
    aria-label="<?php echo esc_attr__('Khối giới thiệu phát triển bền vững', 'pepsico-theme'); ?>">
    ...
    <div class="develop-hero__inner">
        ...
        <a class="develop-hero__btn" href="<?php echo esc_url($btn_link); ?>">
            ...
            <?php if ($btn_icon):
            $allowed = array_merge(wp_allowed_protocols(), ['data']); ?>
            <img class="icon" src="<?php echo esc_url($btn_icon, $allowed); ?>" alt="" loading="lazy" decoding="async"
                sizes="24px">
            <?php endif; ?>
        </a>
        <?php endif; ?>
...
<main id="primary" class="site-main mt-5">
    <div class="container">
        ...
        <section class="sustain" data-gap="28" data-autoplay="5000"
            aria-label="<?php echo esc_attr__('Danh sách sáng kiến bền vững', 'pepsico-theme'); ?>">
            ...
            <button class="s-nav prev" type="button"
                aria-label="<?php echo esc_attr__('Trước', 'pepsico-theme'); ?>">‹</button>
            <button class="s-nav next" type="button"
                aria-label="<?php echo esc_attr__('Sau', 'pepsico-theme'); ?>">›</button>
        </section>
        <?php else: ?>
